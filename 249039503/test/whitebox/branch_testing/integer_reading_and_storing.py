...
class IntegerReadingAndStoringBranchTest(unittest.TestCase):
    def test_let_store_and_read(self):
        code = """
        let x = 10;
        x;
        """
    ...
    def test_reassign_overwrites_value(self):
        code = """
        let x = 10;
        x = 20;
        x;
    ...
    def test_read_used_in_expression(self):
        code = """
        let x = 10;
        let y = 5;
        x + y;
    ...
    def test_reassign_used_in_expression(self):
        code = """
        let x = 1;
        x = x + 2;
        x;
    ...
    def test_read_undefined_variable_raises(self):
        with self.assertRaises(Exception) as ctx:
            run("x;")
        self.assertIn("Undefined", str(ctx.exception))

    def test_assign_undefined_variable_raises(self):
        with self.assertRaises(Exception) as ctx:
            run("x = 1;")
        self.assertIn("Undefined", str(ctx.exception))

    def test_shadowing_in_block_if_supported(self):
        code = """
        let x = 10;
        if (1 < 2) {
            let x = 99;
